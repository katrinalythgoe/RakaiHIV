...
def divergenceX(fastain, patient_id, cutoff):
    # fasta = open('%s' % filename, 'r')
    ...
    for t in range(0, len(sorted_timepoints)):
        ...
        for each in seqs_at_t:
            ...
            # minimum differences to founder strain
            ...
            #print(m_diff, m_diffN, m_diffS, freq)

            divergence.extend([float(m_diff)/float(seq_length)] * freq)
            divergence_N.extend([float(m_diffN)/float(seq_length)] * freq)
            divergence_S.extend([float(m_diffS)/float(seq_length)] * freq)
...
fileout_gag_b = "/Users/jayna/Dropbox/Latency_in_Rakai_ms/Revisions/divergence_at_cp_p24_through_time_BAPS_xx.csv"
...
fileout_gp41_b = "/Users/jayna/Dropbox/Latency_in_Rakai_ms/Revisions/divergence_at_cp_gp41_through_time_BAPS_xx.csv"
...
for p in patients:
    ...
    xx = divergenceX(filein, p, 0.56/100)
